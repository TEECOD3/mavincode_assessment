...
export const useAppNavigation = () => {
  ...
  const isCurrentRoute = useCallback((path: RoutePath | string) => {
...
export const useNavigationGuard = () => {
  ...
  const requireAuth = useCallback((isAuthenticated: boolean, returnUrl?: string) => {
    if (!isAuthenticated) {
  ...
  const requireGuest = useCallback((isAuthenticated: boolean) => {
    if (isAuthenticated) {
      redirectToDashboard();
      return false;
    }
